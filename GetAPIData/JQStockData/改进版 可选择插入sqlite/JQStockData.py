...
# 按照列表获取股票全量数据写CSV(1M 1w 1d 60m 30m 15m 5m 1m)
...
# 获取单一周期数据，入库
def getDataToDB(jqcode, cycle, remain):
    jqcode = jqcode.strip('\n')
    count, total = querylast()
    #获取当前股票的上市日期:聚宽数据，数据格式datetime.date
    starttimestr = jqallstockdata.loc[jqcode, 'start_date']
    starttime = datetime.datetime.strptime(starttimestr,'%Y-%m-%d')
    dict = {'month': '1M', 'week': '1w', 'day': '1d', '60m': '60m'}
    if count > remain:
        if not(cycle.lower() in dict.keys()):
            return False
        print('开始获取数据')
        datajq = getdatajqnew(jqcode, dict[cycle.lower()], starttime, jqmonthheadtime)
        if(datajq.empty != True):
            sqllist = []
            slatehelper = SqliteHelper.sqlite3Helper(r'E:\小程序\Python_Test_del\JQStockDataY.db')
            datajq = datajq.values.tolist()
            for item in datajq:
                sqlstr = """INSERT INTO {9}Data(date,open,close,high,low,volume,money,exchange,symbol) 
                VALUES ({0},{1},{2},{3},{4},{5},{6},'{7}','{8}')""" \
                .format(str(item[0]).replace("-",""),item[1],item[2],item[3],item[4],item[5], \
                    item[6],jqcode.split('.')[1],jqcode.split('.')[0],cycle.lower())
                sqllist.append(sqlstr)
            slatehelper.executeListNoQuery(sqllist)
            slatehelper.releaseconn()
        return True
    else:
        print('帐号剩余的聚宽数据获取次数不足')
        return False
...
def followorder():
    ...
    print('|                   更新股票日期信息表并保存请输入:updateinfo')
    print('|下载列表中股票的某一类型历史数据直到剩余次数少于12500请输入:insert')
    ...
    if order == '1':
        ...
        print('已获取最新股票日期信息列表到jqlist')
        followorder()
        exit()
        return
    elif order == 'insert':
        print('输入说明:月线为month，周线为week，日线为day，60分线为60m，30分线为30m，15分线为15m，5分线为5m，1分线为1m，注意小写')
        cycle = input('请输入你想要手工下载数据的周期:')
        while count > 10000 and len(jqlist) > 0:
            print('--当前文档内第一个代码为%s--' % (jqlist[0]))
            print('++++即将开始获取完整数据的股票为:%s' % (jqlist[0]))
            x = getDataToDB(jqlist[0], cycle, 10000)
            if x == True:
                print('[   股票%s完整数据获取似乎顺利完成了   ]' % (jqlist[0]))
                del jqlist[0]
                writelocallist()
                ...
                print('getDataToDB()获取数据失败,请检查原因,程序退出')
                exit()
                return
        else:
            print('!!!!!!!!聚宽剩余数据获取次数不足!!!!!!!!')
